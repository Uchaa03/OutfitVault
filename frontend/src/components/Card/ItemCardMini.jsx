...
/**
 * ItemCardMini displays a compact card with an image and name.
 *
 * This version uses a <g> element with a transform of translate(37.5, 44)
 * for the image container and sets patternUnits to "userSpaceOnUse".
 * The pattern dimensions have been adjusted to reduce the zoom effect on the image.
 *
 * @component
 * @param {string} name - The name of the item.
 * @param {string} itemImage - The URL for the item's image.
 * @returns {JSX.Element} The rendered ItemCardMini component.
 */
const ItemCardMini = ({ name, itemImage }) => {
  // Generate a unique ID for linking the pattern to the image.
  ...
  return (
    <div className="item-card-mini-container" tabIndex="0" role="button" aria-label={`Item card for ${name}`}>
      ...
      <svg className="item-card-mini-background" width="559" height="445" viewBox="0 0 559 445" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M2.09865 33.2015V49.0664L0.830078 441.551L462.589 445H501.914V441.551V429.825H531.091V418.788H559V398.095V0.092041H147.984H66.7956H52.8414V2.85116V5.61028V19.4059H18.59V33.2015H2.09865Z" fill={darkMode? "#D9D9D9":"#4D4D4D"}/>
      </svg>
      ...
      <svg className="item-card-mini" width="612" height="490" viewBox="0 0 612 490" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <text className="item-card-mini__name" x="50%" y="95%" textAnchor="middle" fontSize="24" fill={darkMode? "#F5F5F5":"#141414"}>{name}</text>
      </svg>
    </div>
  );
};
